...
export async function enrichWebsite(input,env){
 ...
 if(input.analyze&&result.pages.length&&env.AI){
  try{
   ...
   result.analysis={text:response.response,generatedAt:new Date().toISOString(),model:'@cf/meta/llama-3.3-70b-instruct-fp8-fast',sourceUrls:result.pages.map(p=>p.url)};
