...
export function pageSignals(html,url,now=Date.now()){
 ...
 const restricted=/\/accounts\/login|\/login\/?(?:\?|$)|\/checkpoint\//i.test(url)||/log in to (?:continue|see)|login to (?:continue|see)|sign in to (?:continue|see)|enable javascript|verify you are human/i.test(p.text);
...
export async function verifyChannels(input,read=readPublicPage){
 ...
 for(const platform of platforms){const urls=platform==='Website'?[input.website]:[...new Set(result.links.filter(l=>l.platform===platform).map(l=>l.url))].slice(0,2);
